...
// maximum number of digital queues
#define  _MIDI_MAX_DIGI_QUEUES      0x01
...
// file ID for the current digital patch file
#define _SOS_DIGITAL_PATCH_FILE_ID  "HMI-DIGIPATCH102395"
...
// define for the pan controller
#define  _MIDI_PAN_CTRLR                  10
...
// flags for _SOS_DIGITIAL_PATCH_HEADER.wFlags
#define  _WAVETABLE           0x0001
#define  _TRIGGERED            0x0002
#define  _SOS_NO_VELOCITY_SENSING 0x0004

// define for the size of an sos file id
#define  _SOS_FILE_ID_SIZE    0x20

// define for the size of an sos file name
#define  _SOS_FILE_NAME_SIZE  0x20

// digital patch file header
typedef struct _tag_digi_patch_header
{
   // file identifier, "HMI-DIGIPATCH(DATE)"
   BYTE szFileID[ _SOS_FILE_ID_SIZE ];
   
   // name of the patch file
   BYTE szFileName[ _SOS_FILE_NAME_SIZE ];   

   // flags for the file to indicate which systems are present
   // _TRIGGERED
   // _WAVETABLE
   WORD wFlags;

   // offset of wavetable data from start of file, if this element is 0 then
   // there is no wavetable data
   DWORD dwWavetableOffset;

   // offset of triggered data from start of file, if this element is 0 then
   // there is no triggered data
   DWORD dwTriggeredOffset;

   // default triggered patch to use for all undefined patches
   DWORD dwDefaultTriggeredPatch;

   // default wavetable patch to use for all undefined patches
   DWORD dwDefaultWavetablePatch;

   // total wavetable patches
   DWORD dwTotalWavetablePatches;

   // total triggered patches
   DWORD dwTotalTriggeredPatches;

   // array of offsets of the wavetable patch headers
   DWORD dwWavetablePatchHeaders[ _MIDI_DIGI_MAX_INS ];

   // array of offsets of the triggered patch headers
   DWORD dwTriggeredPatchHeaders[ _MIDI_DIGI_MAX_INS ];

   // system extra data
   DWORD dwSystem[ 0x20 ];

} _SOS_DIGI_PATCH_FILE_HEADER;
typedef _SOS_DIGI_PATCH_FILE_HEADER * PSOSDIGIPATCHFILEHEADER;

// structure for a triggered digital patch
typedef struct _tag_sos_digi_triggered_patch
{
   // flags for the patch
   WORD wFlags;
   ...
   // offset of the patch data from the start of the file
   DWORD dwPatchDataOffset;

   // number of bytes of patch data
   DWORD dwPatchDataSize;

   // start loop point from start of patch data, in samples
   DWORD dwOuterLoopStart;

   // end loop point from start of patch data, in samples
   DWORD dwOuterLoopEnd;

   // number of times to loop 
   DWORD dwOuterLoopCount;

   // start loop point from start of patch data, in samples
   DWORD dwInnerLoopStart;

   // end loop point from start of patch data, in samples
   DWORD dwInnerLoopEnd;

   // number of times to loop 
   DWORD dwInnerLoopCount;

   // original sample rate
   DWORD dwSampleRate;

   // number of channels in patch data
   WORD wChannels;

   // format of data, signed/unsigned
   WORD wDCOffset;

   // bits per sample
   WORD wBitsPerSample;

   // pan position of sample
   DWORD dwPanPosition;
   ...
   // offset of next triggered patch header from the start of the file, 
   // if this element is 0 then there are no more triggered patches
   DWORD dwNextTriggeredPatchOffset;

} _SOS_DIGI_TRIGGERED_PATCH;
typedef _SOS_DIGI_TRIGGERED_PATCH * PSOSDIGITRIGGEREDPATCH;
...
// patch entry for the digital patch system
typedef struct _tag_sos_digi_patch_entry
{
   // pointer to the patch data
   PSTR pPatchData;

   // number of owners that the patch has
   WORD wOwners;

   // sample structure to be initialized when the patch is loaded
   _SOS_SAMPLE sSample;

   // patch header
   _SOS_DIGI_TRIGGERED_PATCH sTriggeredPatch;

   // wave table patch header

} _SOS_DIGI_PATCH_ENTRY;
typedef _SOS_DIGI_PATCH_ENTRY * PSOSDIGIPATCHENTRY;

// structure for a DIGITAL MIDI driver
typedef struct
{
   // flags
   // _SOS_ACTIVE
   ...
   // patch file header
   _SOS_DIGI_PATCH_FILE_HEADER sPatchHeader;

   // pointer to the patch list
   PSOSDIGIPATCHENTRY psPatchList;

   // name of the patch file to use
   BYTE szPatchFile[ _MAX_DIR ];

   // handle for the digital driver
   HSOS   hDIGIDriver;

   // handle for the queue system
   HSOS   hQueue;

} _MIDI_DIGI_SYSTEM;
typedef _MIDI_DIGI_SYSTEM * PMIDIDIGISYSTEM;
...
} _SOS_DIGI_QUEUE_ELEMENT;

// structure for the DIGITAL MIDI queue system
typedef struct
{
   // flags
   WORD  wFlags;
   ...
   // maximum number of available voices
   WORD  wMaxVoices;

   // number of used voices
   WORD  wUsedVoices;

   // element for each voice in the queue
   _SOS_DIGI_QUEUE_ELEMENT sElement[ _SOS_MAX_VOICES ];
...
{  
   ...
   _SOS_MIDI_DIGI_DRIVER sDIGIInit;
